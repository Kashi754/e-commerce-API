...
// environment these reqest bodies need to be changed to represent the
// given function parameters.

export async function fetchShippingServiceTypes(cart, zip, accessToken) {
  ...
  const visibleServices = shippingServices.output.packageOptions.filter(
    (service) => {
      return (
        ...
        (service.maxDeclaredValue?.amount >= cart.totalPrice || !service.maxDeclaredValue)
      );
    }
  );
...
export async function fetchTransitTimes(cart, zip, accessToken, serviceTypes) {
  ...
  const transitTimeDetails = transitTimes.output.transitTimes[0].transitTimeDetails;
  const filteredTransitTimes = transitTimeDetails.filter(
    (transitTime) => {
      return serviceTypes.indexOf(transitTime.serviceType) >= 0;
    }
  );
  const paredTransitTimes = filteredTransitTimes.map(
    (service) => {
      return {
...
export async function fetchShippingRates(cart, zip, accessToken, serviceTypes, rateType) {
  // Fetch transit times from FEDEX API
  const url = 'https://apis-sandbox.fedex.com';
...
export async function createNewShipment(cart, customerAddress, accessToken, serviceTypes) {
  // Fetch transit times from FEDEX API
  const url = 'https://apis-sandbox.fedex.com';
  ...
  const paredShipment = {
    ...
    expected_delivery_day: newShipment.completedShipmentDetail.operationalDetail.deliveryDay,
    expected_delivery_date: newShipment.completedShipmentDetail.operationalDetail.deliveryDate,
  }
