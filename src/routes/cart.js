...
const { fetchShippingServiceTypes, fetchTransitTimes, fetchShippingRates } = require('../utilities/shippingUtilities');
...
cartRouter.post('/shipping', async (req, res, next) => {
    ...
    const transitTimes = await fetchTransitTimes(cart, zip, accessToken, serviceTypes);
    // Returns { serviceType, totalCharge }
    const shippingRates = await fetchShippingRates(cart, zip, accessToken, serviceTypes, 'LIST');
