  ...
  try {
    ...
    if (!cartId) {
      const error = new Error(
        `Cart with payment_intent: ${paymentIntentId} not found!`
      );
      return [null, error];
    }
...
exports.addItemToCart = async (cartId, itemData, done) => {
  try {
    ...
    const cartItem = await knex('cart_product').returning('product_id').insert({
      cart_id: cartId,
...
exports.editCartProduct = async (cartId, itemData, done) => {
  try {
    ...
    const cartItem = await knex('cart_product')
      ...
      .update({ quantity: qty }, ['cart_id', 'product_id']);
...
exports.createPaymentIntent = async (cartId, paymentIntent, done) => {
  try {
    const intentCreated = await knex('cart').where('id', cartId).update(
      {
...
exports.updateShippingPrice = async (cartId, shippingPrice, done) => {
  try {
    const serviceTypeUpdated = await knex('cart').where('id', cartId).update(
      {
